...
DefinitionsElement.prototype.addChild = function(child) {
    ...
    if (child instanceof TypesElement) {
        ...
        //console.log(child.schemas[ 'http://tempuri.org/Imports'].includes);
    }
    ...
    else if(child.name == 'documentation'){
        return;
    }
    else if (child instanceof TypesElement) {
        self.schemas = child.schemas;
    }
    else if (child.nsName === 'wsdl:documentation') {
    	 return;
    	    }
    else if(child.nsName === 'wsp:Policy'){
    	 return;
    }
 else if(child.nsName === 'wsdl:import'){
	 return;
    }
    else {
...
MessageElement.prototype.postProcess = function(definitions) {
    ...
    if (part.$element) {
       ...
       // console.log(definitions);
        if(!definitions.schemas[definitions.xmlns[ns]])
        	 this.element={};
        else
...
var WSDL = function(definition, uri, options) {
    ...
    process.nextTick(function() {
    	//console.log("TEST:definition-1",definition);
...
WSDL.prototype._processNextInclude = function(includes, callback) {
    ...
    debugger;
   //console.log("TEST URL 8:",self.uri);
    var includePath;
...
function open_wsdl(uri, options, callback) {
    ...
    //HTTP WEB 服务
    	...
    	//console.log("TEST-URL地址6-2:",uri);
        http.request(uri, null /* options */, function (err, response, definition) {
        	//console.log("TEST-definition-6-2:",definition);
            if (err) {
                callback(err);
            }
