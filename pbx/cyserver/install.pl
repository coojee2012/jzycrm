...
{
print qq~
  Vsiosoft Install Stage by hoowa sun $VERSION

syntax:
  install VsionSoft  :    ./install.pl --install
  setup VsionSoft    :    ./install.pl --setup
  uninstall VsionSoft:    ./install.pl --uninstall
...
my	%instvar;
	...
	#测试是否已经安装了
	&println('error',"Your need to remove your old install, try to use 'install.pl --unistall'") if (-d "/freeiris2/" || -e "/etc/init.d/hardware" || -e "/etc/init.d/fri2d");
