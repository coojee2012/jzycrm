...
var queue = require('../modules/ippbx/queue');
...
routing.prototype.queue = function (queuenum, assign, callback) {
    ...
    async.auto({
        ...
        getQueueMember: function (cb, results) {
            queue.findOne({
                where: {
                ...
                if (err)
                    cb(err, inst);
                if (inst != null && inst.members !== "") {
                    var members = inst.members.substr(0, inst.members.length - 1).split('&');
                    cb(null, members);
                } else {
        ...
        hasCanUse: ["stopwater","findDnd", "findBusyCallee","findBusyCaller", function (cb, results) {
            var less = arrayNo(results.getQueueMember, results.findDnd);
            less = arrayNo(less, results.findBusyCallee);
            less = arrayNo(less, results.findBusyCaller);
            if(less.length>0){
                cb(null,less);
            }else{
                context.Playback('queue-periodic-announce', function (err, response) {
                    cb("播放完了引发挂机", response);
                });
            }
        }],
        queue: ['hasCanUse',
