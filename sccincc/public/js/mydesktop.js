...
myLib.desktop.win = {
    ...
    newWin: function(options) {
        ...
        var defaults = {
            ...
            /* Posible are pixels or 'center' */
            WindowWidth: Math.round(wh['w'] * 0.88),
            /* Only pixels */
            WindowHeight: Math.round(wh['h'] * 0.78),
            /* Only pixels */
            WindowMinWidth: 250,
            /* Only pixels */
            WindowMinHeight: 250,
        ...
        if ($("#" + id).size() <= 0) {
            ...
            //如果已存在此窗口，判断是否隐藏
            var wins = $("#" + id),
                objTab = myLib.desktop.taskBar.findWinTab(wins);
            if (wins.is(":hidden")) {
                wins.show();
...
myLib.desktop.lrBar = {
    upLrBar: function() {
        var myData = myLib.desktop.getMydata(),
            ...
            wh = myData.winWh;
        $lrBar.css({
            'top': Math.floor((wh['h'] - $lrBar.height()) / 2) - 60
        });
                        ...
                        success: function(ajaxdata, textStatus) {
                            if (ajaxdata.Response == "Error") {
                                ...
                                if (_DIDSTATUS == "off") {
                                    ...
                                    _DIDSTATUS = "off";
                                    $("#app0img").attr("src", "/themes/default/icon/leftmenubtn1.png");
                                    alert("取消示忙成功！");
                                }
                    ...
                    $.ajax({
                        ...
                        success: function(ajaxdata, textStatus) {
                            if (ajaxdata.response == "Error" && _PARKSTATUS == 0) {
                                ...
                                _PARKSTATUS = 0;
                                $("#app2img").attr("src", "/themes/default/icon/leftmenubtn3.png");
                                alert("取回保持电话失败，请及时主动回呼来电！");

                            } else if (ajaxdata.response == "Success" && _PARKSTATUS == 0) {
                                _PARKSTATUS = 1;
                                $("#app2img").attr("src", "/themes/default/icon/leftmenubtn2-1.png");
