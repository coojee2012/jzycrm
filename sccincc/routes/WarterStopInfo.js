...
exports.get = function(req, res) {
	...
	res.render('WarterStopInfo/index.html', {
		title: '停水记录列表',
		where: where
	});
};
...
exports.createget = function(req, res) {
	...
	var inst = null;
	res.render('WarterStopInfo/create.html', {
		title: '新增停水记录',
		inst: inst,
		msg: null,
		util: util
		...
		if (err) {
			...
			WarterStopInfo_mod.isValid(function(valid) {
				if (!valid) {
					...
					res.render('WarterStopInfo/create.html', {
						title: '新增停水记录',
						inst: WarterStopInfo_mod,
						msg: WarterStopInfo_mod.errors
					});
				} else {
...
exports.editget = function(req, res) {
	...
	DbMode.findOne({
		...
		if (err) {
			syslog.add(req, res, 'sql', err);
			res.render('WarterStopInfo/edit.html', {
				title: '编辑停水记录',
				inst: null,
				msg: err,
				util: util
...
exports.detail = function(req, res) {
	...
	DbMode.findOne({
		...
		if (err) {
			syslog.add(req, res, 'sql', err);
			res.render('WarterStopInfo/detail.html', {
				title: '停水记录详细',
				inst: null,
				msg: err,
				util: util
