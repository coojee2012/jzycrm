...
exports.get = function(req, res) {
	...
	where['uDepId'] = -1;
	where['uRolyId'] = -1;
	res.render('UserInfo/index.html', {
		title: '用户列表',
		where: where
	});
};
...
exports.createget = function(req, res) {
	...
	var inst = null;
	res.render('UserInfo/create.html', {
		title: '新增用户',
		inst: inst,
		msg: null,
		util: util
...
exports.EngineerList = function(req, res) {
	...
	where['uRolyId'] = 9;
	res.render('UserInfo/engineerList.html', {
		title: '工程师列表',
		where: where
	});
}
...
exports.createEngineerGet = function(req, res) {
	var inst = null;
	res.render('UserInfo/createEngineer.html', {
		title: '新增工程师',
		inst: inst,
		msg: null,
		util: util
...
exports.createEngineerPost = function(req, res) {
	...
	UserInfo_mod.uRolyId = 9;
	UserInfo_mod.uPass = '11111111';
	...
	UserInfo_mod.isValid(function(valid) {
		if (!valid) {
			...
			DbMode.findOne({
				...
				if (inst === null)
					UserInfo_mod.uExten = '8001';
				else {
					if(UserInfo_mod.isAgent === 1){
						UserInfo_mod.uExten = (parseInt(inst.uExten) + 1).toString();
					}else{
					UserInfo_mod.uExten='8801';	
					}
...
exports.editEngineerGet = function(req, res) {
	...
	DbMode.findOne({
		...
		if (err) {
			res.render('UserInfo/editEngineer.html', {
				title: '编辑工程师',
				inst: null,
				msg: err,
				util: util
		...
		if (err) {
			...
			for (var key in req.body) {
				...
				if (DbMode['cloums'][key] != null && DbMode['cloums'][key]['input']['type'] === 'password') {
					...
					inst[key] = jiaMi(req.body[key]);
					console.log(req.body[key] + "MD5:" + inst[key]);
					continue;
				}
				inst[key] = req.body[key];
			}
...
exports.detail = function(req, res) {
	...
	DbMode.findOne({
		...
		if (err) {
			res.render('UserInfo/detail.html', {
				title: '用户详细',
				inst: null,
				msg: err,
				util: util
