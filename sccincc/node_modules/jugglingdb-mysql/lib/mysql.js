...
MySQL.prototype.all = function all(model, filter, callback) {
    ...
    if (filter) {
    	...
    	/*if(filter.jin){
    	sql = buildJin(filter.jin,this.tableEscaped(model));
    		
    	}else{
    		sql =	'SELECT a.* FROM ' + this.tableEscaped(model)+ ' a ';	
    	}*/
...
/*else{
   ...
   // console.log(sql);
    return sql;
    function buildJin(joins,dbname){
    	var strdb='';
    	var strsl='';
    	Object.keys(joins).forEach(function(key){    		
    		strdb+=' left join '+joins[key].dbname+' '+key +' on a.'+joins[key].dbcl+' = '+key+'.id ';    		    	
    		joins[key].cloums.forEach(function(cl){
    			strsl+=','+key+'.'+cl;	
    		});
    		
    		
    	});
    	return 'SELECT a.* '+strsl+' FROM '+dbname +' a '+ strdb
    }
