...
function dateToMysql(val) {
    //console.log('mysqltime:',val);
    // return val.getUTCFullYear() + '-' +
    //     fillZeros(val.getUTCMonth() + 1) + '-' +
    //     fillZeros(val.getUTCDate()) + ' ' +
    ////     fillZeros(val.getUTCHours()+8) + ':' +
    //     fillZeros(val.getUTCMinutes()) + ':' +
    //    fillZeros(val.getUTCSeconds());
    if (val instanceof Date) {} else {
        //val = val.replace(/-/g,"/"); 
        val = new Date('2000/01/01 00:00:00');
    }
...
MySQL.prototype.count = function count(model, callback, filter) {
    var sql = 'SELECT count(*) as cnt2 FROM ' + this.tableEscaped(model);
    var self = this;
    var props = this._models[model].properties;
    console.log(filter);
    if (filter) {
    ...
    this.query(sql, function(err, data) {
        ...
        //console.log(data);
        return callback(null, data[0].cnt2);
